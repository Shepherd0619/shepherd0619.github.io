---
title: "Jenkins与自定义命令行参数"
date: 2024-01-15
source: https://blog.csdn.net/u012587406/article/details/135604418
categories: Unity 开发
tags: [Unity, Jenkins, CI/CD, 命令行]
---
...
当然有些时候我们需要自己定义一个或者多个命令行参数，比如我们要启动一个服务器构建、接入一个CI/
CD 
，人手动操作肯定费时费力，而且不熟悉Unity编辑器的新手难免会出错。
...
1. -batchmode：这个参数可以让Unity在无需用户交互的情况下启动并执行指定的任务。这对于自动化构建和
持续集成
非常有用。
...
![](images/25bd1578c11f.png)

然后在拉起Unity编辑器那块引用上面的参数，如${上面在Jenkins定义的参数名}

![](images/280560a80a4e.png)

保存设置后，以后执行job都会让你配置参数了

![](images/44bad8c2b84e.png)

## 总结

总结起来，Unity命令行参数是一种非常有用的工具，可以帮助开发者更好地管理和调试项目。通过使用命令行参数，我们可以在启动Unity时自定义一些行为和设置，从而提高开发效率和减少错误。希望本文对大家了解和使用Unity命令行参数以及如何自定义有所帮助。